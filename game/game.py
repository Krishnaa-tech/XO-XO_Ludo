...
#---------------------------------------------------------------------------------------------------
# Caption & icon
pygame.display.set_caption("XO-XO")
...
def gameDone(player_pawns):
    destination_area = (4, 4)

    # Check if all player pawns are in the destination area
    for pawn in player_pawns:
        if pawn.Position != destination_area:
            return False  # one pawn atleast has not reached the destination
